...
class LightOpenID
{
  ...
  function __construct($host)
  {
    ...
    $uri = rtrim(
      preg_replace('#((?<=\?)|&)openid\.[^&]+#', '', $_SERVER['REQUEST_URI']),
      '?'
    );
  ...
  function __get($name)
  {
    switch ($name) {
      ...
      case 'mode':
        return empty($this->data['openid_mode'])
          ? null
          : $this->data['openid_mode'];
    }
  }
  ...
  protected function request_curl(
    $url,
    $method = 'GET',
    $params = [],
    $update_claimed_id
  ...
  protected function request_streams(
    $url,
    $method = 'GET',
    $params = [],
    $update_claimed_id
    ...
    if (isset($http_response_header)) {
      $this->headers = $this->parse_header_array(
      ...
      if ($yadis) {
        ...
        if (
          ...
          foreach ($m[1] as $content) {
            ...
            if (
              preg_match(
                '#<Type>\s*' . $ns . '(server|signon)\s*</Type>#s',
                $content,
                $type
              )
            ) {
        ...
        if (isset($this->headers['x-xrds-location'])) {
          $url = $this->build_url(
            parse_url($url),
            parse_url(trim($this->headers['x-xrds-location']))
          );
          continue;
        }

        $location = $this->htmlTag(
          $content,
          'meta',
          'http-equiv',
          'X-XRDS-Location',
  ...
  protected function sregParams()
  {
    ...
    if ($this->required) {
      ...
      $params['openid.sreg.required'] = implode(
        ',',
        $params['openid.sreg.required']
      );
    }
  ...
  protected function getAxAttributes()
  {
    ...
    if (
      ...
      foreach ($this->data as $key => $val) {
        if (
          substr($key, 0, strlen('openid_ns_')) == 'openid_ns_' &&
